...
func TestClient_ListImages(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			client := NewClient(logger, mockExec, readWriter)
			got, err := client.ListImages(context.Background())
			if tc.wantErr {
				require.Error(err)
				require.Nil(got)
			} else {
...
func TestClient_ListArtifacts(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.setupMock(mockExec)
			client := NewClient(logger, mockExec, readWriter)
			got, err := client.ListArtifacts(context.Background())
			if tc.wantErr {
				require.Error(err)
				require.Nil(got)
			} else {
...
func TestClient_RemoveImage(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.setupMock(mockExec)
			client := NewClient(logger, mockExec, readWriter)
			err := client.RemoveImage(context.Background(), tc.image)
			if tc.wantErr {
				require.Error(err)
			} else {
				require.NoError(err)
...
func TestClient_RemoveArtifact(t *testing.T) {
	require := require.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.setupMock(mockExec)
			client := NewClient(logger, mockExec, readWriter)
			err := client.RemoveArtifact(context.Background(), tc.artifact)
			if tc.wantErr {
				require.Error(err)
			} else {
				require.NoError(err)
