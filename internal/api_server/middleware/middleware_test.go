...
func (m *mockOrgStore) CreateMany(_ context.Context, orgs []*model.Organization) ([]*model.Organization, error) {
	return nil, m.respErr
}
...
func TestAddOrgIDToCtx(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			resolver := resolvers.NewExternalResolver(storeMock, 5*time.Minute, &providers.ClaimsProvider{}, logrus.New())
