...
// -----------------------------------------------------------------------------
// Tests for the query extractor and ExtractOrgIDToCtx middleware.
// -----------------------------------------------------------------------------
func TestExtractOrgIDFromRequestQuery(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:               "no mapped identity returns error",
			rawQuery:           "",
			userOrgs:           nil,
			hasMappedIdentity:  false,
			wantPresent:        false,
			wantMiddlewareErr:  flterrors.ErrMissingMappedIdentity,
			wantMiddlewareCode: http.StatusInternalServerError,
			wantContextID:      uuid.Nil,
		},
		{
			name:              "no param with zero orgs uses default org",
			rawQuery:          "",
			userOrgs:          []*orgmodel.Organization{},
			hasMappedIdentity: true,
			wantPresent:       false,
			wantMiddlewareErr: nil,
			wantContextID:     org.DefaultID,
		},
		{
		...
		{
			name:               "empty param with no mapped identity returns error",
			rawQuery:           "org_id=",
			userOrgs:           nil,
			hasMappedIdentity:  false,
			wantPresent:        false,
		...
		{
			name:              "valid id with mapped identity",
			rawQuery:          fmt.Sprintf("org_id=%s", orgID),
			userOrgs:          []*orgmodel.Organization{},
			hasMappedIdentity: true,
			wantPresent:       true,
		...
		{
			name:              "default org id is present and not overridden",
			rawQuery:          fmt.Sprintf("org_id=%s", org.DefaultID),
			userOrgs:          []*orgmodel.Organization{{ID: orgID, ExternalID: "user-org"}},
			hasMappedIdentity: true,
			wantPresent:       true,
			wantMiddlewareErr: nil,
			wantContextID:     org.DefaultID, // should NOT be overridden by orgID
		},
		{
			name:               "invalid uuid",
			...
			hasMappedIdentity:  false,
			wantPresent:        false,
			wantMiddlewareErr:  flterrors.ErrInvalidUUID,
			wantMiddlewareCode: http.StatusBadRequest,
			wantContextID:      uuid.Nil,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			middleware := ExtractOrgIDToCtx(QueryOrgIDExtractor, logger)
			...
			if tc.wantMiddlewareErr != nil {
				...
				assert.Contains(t, rr.Body.String(), tc.wantMiddlewareErr.Error(),
					"expected response body to contain %q", tc.wantMiddlewareErr.Error())
				return
			}
