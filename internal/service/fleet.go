...
func (h *ServiceHandler) CreateFleet(ctx context.Context, orgId uuid.UUID, fleet api.Fleet) (*api.Fleet, api.Status) {
	...
	h.CreateEvent(ctx, orgId, GetResourceCreatedOrUpdatedEvent(ctx, true, api.FleetKind, *fleet.Metadata.Name, status, nil))
...
func (h *ServiceHandler) ReplaceFleet(ctx context.Context, orgId uuid.UUID, name string, fleet api.Fleet) (*api.Fleet, api.Status) {
	...
	h.CreateEvent(ctx, orgId, GetResourceCreatedOrUpdatedEvent(ctx, created, api.FleetKind, name, status, &updateDesc))
	return result, status
}
...
func (h *ServiceHandler) DeleteFleet(ctx context.Context, orgId uuid.UUID, name string) api.Status {
	...
	if deleted || err != nil {
		h.CreateEvent(ctx, orgId, GetResourceDeletedEvent(ctx, api.FleetKind, name, status))
	}
	return status
}
...
func (h *ServiceHandler) PatchFleet(ctx context.Context, orgId uuid.UUID, name string, patch api.PatchRequest) (*api.Fleet, api.Status) {
	...
	status := StoreErrorToApiStatus(err, false, api.FleetKind, &name)
	h.CreateEvent(ctx, orgId, GetResourceCreatedOrUpdatedEvent(ctx, false, api.FleetKind, name, status, &updateDesc))
	return result, status
}
