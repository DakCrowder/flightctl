	...
	tests := []struct {
	...
	}{
		{
			...
			identity: &mockTokenIdentity{
				claims: map[string]interface{}{
					OrganizationClaimName: map[string]string{
						"org1": "id1",
						"org2": "id2",
						"org3": "id3",
					},
		...
		},
		{
			...
			want:     nil,
			wantErr:  true,
			errMsg:   "cannot get organizations claims from a non-token identity",
		},
		{
			...
			want:    nil,
			wantErr: true,
			errMsg:  "unable to get organizations claims from token identity",
		},
		{
			...
			want:    nil,
			wantErr: true,
			errMsg:  "invalid organizations claims format",
		},
		{
