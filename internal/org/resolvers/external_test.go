...
type mockOrgStore struct {
	...
	createManyResult    []*model.Organization
	createManyCallCount int
...
func (m *mockOrgStore) CreateMany(ctx context.Context, orgs []*model.Organization) ([]*model.Organization, error) {
	m.createManyCallCount++
	return m.createManyResult, m.createManyError
}
...
func TestExternalResolver_getOrg(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resolver := NewExternalResolver(tt.store, tt.ttl, nil, logrus.New())
...
func TestExternalResolver_GetUserOrganizations(t *testing.T) {
	...
	test := []struct {
		...
		expectedCreateManyCallCount        int
	...
	}{
		{
			...
			expectedCreateManyCallCount:        0,
			expectedResult:                     []*model.Organization{},
			expectedErrorString:                "",
		},
		{
			name: "all orgs are in the store",
			store: &mockOrgStore{
				listByExternalIDsResult: []*model.Organization{
		...
		{
			name: "one org in store, one org not in store",
			store: &mockOrgStore{
				listByExternalIDsResult: []*model.Organization{
					{ID: orgID, ExternalID: "1", DisplayName: "Test Org 1"},
				},
		...
		{
			name: "error from store create call",
			store: &mockOrgStore{
				createManyError: errors.New("error from store create call"),
			},
			externalOrgProvider: &mockExternalOrgProvider{
				orgs: []org.ExternalOrganization{
					{ID: "1", Name: "Test Org 1"},
			...
			expectedCreateManyCallCount:        1,
			expectedResult:                     nil,
			expectedErrorString:                "error from store create call",
		},
	}

	for _, tt := range test {
		t.Run(tt.name, func(t *testing.T) {
			resolver := NewExternalResolver(tt.store, 0, tt.externalOrgProvider, logrus.New())
			defer resolver.Close()
			...
			assert.Equal(t, tt.expectedCreateManyCallCount, tt.store.createManyCallCount)
